    ...
    private func readInEnemies() {
        ...
        if (enemies.count > 0) {
            for i in 0...(enemies.count-1) {
                if enemies[i].enemyType == "turtle" {
                    enemyArray.append(TurtleSprite(x: rect.midX+CGFloat(enemies[i].enemyX), y: zeroY + CGFloat(enemies[i].enemyY)))
                } else if enemies[i].enemyType == "donkeykong" {
                    enemyArray.append(GorillaSprite(x: rect.midX+CGFloat(enemies[i].enemyX), y: zeroY + CGFloat(enemies[i].enemyY)))
                }
            }
        }
    }
    ...
    func readInPresets() {
        ...
        if (presets.count > 0) {
            for i in 0...(presets.count-1) {
                if presets[i].presetType == "turtle_platform" {
                    presetArray.append(TurtlePlatformHelper(x: rect.midX+CGFloat(presets[i].presetX), y: rect.midX+CGFloat(presets[i].presetY), size: presets[i].presetSize))
